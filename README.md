...
通过 uprobe 技术监控 HTTPS 加密流量：

- **SSL uprobe 挂载**：支持 OpenSSL、Node.js 等 SSL 库
- **HTTPS 解密观测**：捕获加密流量中的明文数据
- **LLM API 解析**：自动解析 HTTPS 中的 LLM 请求/响应
- **进程级过滤**：支持按进程名过滤目标应用

### TOOLCALL 工具调用监控

监控 OpenClaw 智能体的工具/技能调用：

- **工具名称识别**：记录调用的工具/技能名称
- **参数捕获**：记录工具调用的输入参数（JSON 格式）
- **结果追踪**：记录工具执行结果摘要
- **风险等级**：支持按风险等级过滤工具调用
- **对话关联**：工具调用与对话上下文关联
...
| 参数 | 示例 | 说明 |
|------|------|------|
| **SSL 库路径** | `/usr/bin/node` | SSL 库或可执行文件路径 |
| **目标进程名过滤** | `openclaw` | 只监控指定进程的 SSL 流量 |
| **调试输出** | `y/N` | 启用 SSL 明文数据打印 |

> 💡 **提示**：Node.js 应用可指定 node 可执行文件路径；Python 应用可指定 libssl.so 路径。

#### 2.10 TOOLCALL 监控配置

启用并配置 OpenClaw 工具调用监控：
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| **风险等级过滤** | `HIGH,MEDIUM,LOW,INFO` | 按风险等级过滤工具调用 |
| **最大参数长度** | `2048` | 参数 JSON 最大长度（字节） |
| **最大结果长度** | `2048` | 结果摘要最大长度（字节） |
| **最大记录数** | `1000` | 每对话最大工具调用记录数 |
| **调试输出** | `y/N` | 启用工具调用调试输出 |

> 💡 **提示**：TOOLCALL 监控针对 OpenClaw 智能体的工具/技能调用进行深度观测。
